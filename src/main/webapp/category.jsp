...
String sqlTot="select count(*) total from product where mainCategory='"+main+"' and subCategory='"+sub+"'";	
...
String sqlPro = "select * from product where mainCategory='"+main+"' and subCategory='"+sub
	+"' order by id desc limit "+index_no+","+unit;
...
while(rs2.next()) {
	...
	String files=rs2.getString("file");
%>
<div class="clearfix" style="margin:40px 0">
<div style="padding:0 100px">
<div style="float:left;"><img src="./upload/<%=files%>" width="100px"></div>
	...
	<ul class="number">
		...
		<c:if test="${(startNum+i) <= lastNum}">
			<li><a style="${(page==(startNum+i))?'color:#c0392b':''}" 
				href="category.jsp?p=${startNum+i}&main=<%=main%>&sub=<%=sub%>">${startNum+i}</a></li>
		</c:if>
		</c:forEach>	
	</ul>
	<div class="next">
